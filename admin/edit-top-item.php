	...
	
{
  ...
  // Check if file already exists
    ...
    $msg= "Sorry, there was an error uploading your file.";
		...
		$top_image=$_POST["old_top_value"];
	}
	
	$query="UPDATE `top-item` SET title='$title',top_image='$top_image',modified_on='$modified_on' where  ID='$edit_id'";
...
?>
  <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Top Food Item</h1>
      </div>
	  <div class="container my-5 form">
		...
		$edit_id=$_GET['editid'];
		$ret=mysqli_query($con,"select * from `top-item` where ID=$edit_id");
		...
		<div class="form-group">
			  ...
			  <input type="file" class="form-control" id="top_image" onchange="return fileValidation()" placeholder="Enter top Image" name="top_image"/>
			<div class="validation"></div>
			</div>
			<?php
			if($row['top_image'])
			...
			?>
			<img src="<?php echo 'uploads/top-item/'.$row['top_image']?>" alt="not found" height="300px" width="400px"/>
			<input type="hidden" name="old_top_value" value="<?php echo $row["top_image"]?>"><br>
			<?php
			}
			?>
